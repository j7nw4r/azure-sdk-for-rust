...
- Fixed a deadlock when a CBS failure during management-client creation triggered connection recovery. The management-client cache held its lock across the whole build. The build authorizes the `$management` path, and a link or connection failure there runs the recovery hook, which clears that same cache from the same task. `async_lock::Mutex` is not reentrant, so the client hung instead of reporting the error. The management client now uses the same lock-free `OnceCell` cache as the sender, session, and receiver paths, so the build runs with no lock held and recovery can replace the cache without waiting for a build in flight. ([#4728](https://github.com/Azure/azure-sdk-for-rust/issues/4728))
